...
> = ({
  ...
  const [turnover, setTurnover] =
    useState(profile.turnover);

  const [location, setLocation] =
    useState(profile.location);

  const [employees, setEmployees] =
    useState(profile.employees);
  ...
  /*
   * IMPORTANT:
   * Whenever the selected business changes,
   * refresh the Settings form with that business.
   */
  useEffect(() => {
  ...
  const integrations = [
    {
      name: 'GSTN & E-Way Portal',
      desc: 'Direct GSTR-1, 2B & 3B automated reconciliation',
      status: 'Connected',
    ...
    },
    {
      ...
      desc: 'Bank statement fetch via Setu / Finvu protocol',
      status: 'Connected',
      lastSync: '1 hour ago',
      active: true
    },
    {
      ...
      desc: 'Syncs ledger accounts, receivables & inventory',
      status: 'Connected',
      lastSync: 'Today, 09:30 AM',
      active: true
    },
    {
      name: 'Zoho Books & Busy ERP',
      desc: 'Real-time sales invoices and voucher streaming',
      status: 'Available',
      lastSync: 'Not synced',
      active: false
    },
    {
      name: 'TReDS Settlement (RXIL / Invoicemart)',
      desc: 'Automated invoice discounting auction bridge',
      status: 'Connected',
      lastSync: 'Yesterday',
      active: true
    }
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-purple-950/30 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-purple-300">
            ...
            <span>
              ENTERPRISE CONFIGURATION & DATA SYNC
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 p-6 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-4 shadow-xl">
          ...
          <div className="p-3 bg-purple-950/30 border border-purple-500/20 rounded-xl text-xs text-purple-300 flex items-center gap-2">
            ...
            <span>
              256-bit encrypted bank-grade telemetry stream with zero credential storage.
            </span>
