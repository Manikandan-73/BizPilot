...
export const ProblemSection: React.FC = () => {
  ...
  return (
    <section className="py-12 sm:py-20 relative bg-[#0F1219] border-y border-[#222936] min-w-0">
      <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 min-w-0">
        ...
        {/* Big Counter Banner */}
        <div className="mb-10 sm:mb-14 p-4 sm:p-8 rounded-2xl bg-[#121722] border border-[#222936] text-center relative overflow-hidden shadow-lg min-w-0">
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-around gap-6">
            <div className="space-y-1">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-black text-violet-400">
                63 Million+
              </div>
              <div className="text-xs sm:text-sm font-semibold text-[#F8FAFC]">MSMEs Operating in India</div>
              <p className="text-[11px] sm:text-xs text-[#707A8C] max-w-xs mx-auto">Contributing ~30% to India's GDP and 45% to total exports</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="text-xs sm:text-sm font-semibold text-[#F8FAFC]">Access to Formal Bank Credit</div>
              <p className="text-[11px] sm:text-xs text-[#707A8C] max-w-xs mx-auto">Over 85% remain underbanked or rely on expensive non-institutional loans</p>
            </div>

            <div className="h-12 w-px bg-[#222936] hidden md:block"></div>

            <div className="space-y-1">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-black text-amber-400">
                42 Days
              </div>
              <div className="text-xs sm:text-sm font-semibold text-[#F8FAFC]">Average Loan Processing Delay</div>
              <p className="text-[11px] sm:text-xs text-[#707A8C] max-w-xs mx-auto">Manual paperwork, multiple branch visits, and opaque criteria</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 min-w-0">
          {problems.map((p, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="p-4 sm:p-6 rounded-2xl bg-[#121722] border border-[#222936] hover:border-[#303848] transition-all hover:translate-y-[-2px] group space-y-4 flex flex-col justify-between shadow-sm min-w-0"
              >
                <div className="space-y-3">
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-[#F8FAFC] group-hover:text-violet-400 transition-colors">
                    {p.title}
                  </h3>
                ...
                <div className="pt-3 border-t border-[#222936] flex items-baseline justify-between">
                  <span className="text-lg sm:text-xl font-extrabold text-rose-400">{p.stat}</span>
                  <span className="text-[10px] sm:text-[11px] text-[#707A8C] font-medium">{p.statLabel}</span>
                </div>
              </div>
            );
          })}
          ...
          {/* Callout box */}
          <div className="p-4 sm:p-6 rounded-2xl bg-gradient-to-br from-[#161C27] to-[#121722] border border-violet-500/30 flex flex-col justify-between space-y-4 shadow-sm min-w-0">
